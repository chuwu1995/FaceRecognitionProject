...
//		System.out.println(name + " visit the reception at " + date);
		...
		int x = dbManager.getUserDAO().insertUser("XiXi", "Female", "MISM");
		System.out.println(x);
//		dbManager.getRecordDAO().insertRecord(1000000, "Chatting");
		ArrayList<User> ul = dbManager.getUserDAO().getAllUser();
		
		for(User u: ul){
			System.out.println(u.getUID().getValue() + u.getName().getValue());
